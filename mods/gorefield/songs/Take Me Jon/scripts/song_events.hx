...
function postCreate()
{
    ...
    if (FlxG.save.data.vhs && Options.gameplayShaders)
    {
    ...
    if (FlxG.save.data.static && Options.gameplayShaders)
    {
...
function tweenCamera(in:Bool){
	...
	FlxTween.tween(camCharacters,{x: in ? 0 : -900},(Conductor.stepCrochet / 1000) * 5, {ease: FlxEase.quadInOut});
