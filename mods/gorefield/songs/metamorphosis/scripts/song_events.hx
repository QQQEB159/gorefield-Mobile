...
function postCreate() {
    ...
    if (FlxG.save.data.drunk && Options.gameplayShaders) stage.stageSprites["BG"].shader = drunkShader;
	...
	if (FlxG.save.data.static && Options.gameplayShaders) FlxG.camera.addShader(staticShader);
    ...
    for (i=>strum in strumLines.members) {
        ...
        for (char in strum.characters) {
			...
			if (FlxG.save.data.wrath && Options.gameplayShaders) char.shader = newShader;
		}
    }
    
    mobileControls.instance.buttonLeft.color = 0xFF7C2E56;
    mobileControls.instance.buttonDown.color = 0xFF9D2212;
    mobileControls.instance.buttonUp.color = 0xFF9D2212;
    mobileControls.instance.buttonRight.color = 0xFF7C2E56;
}
