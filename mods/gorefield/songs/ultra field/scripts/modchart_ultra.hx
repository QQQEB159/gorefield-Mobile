...
function create() {
    ...
    if (FlxG.save.data.warp && Options.gameplayShaders) {ultraCam.addShader(chromaticWarpShader); camGame.addShader(chromaticWarpShader2); camGame.addShader(chromatic);}
    if (FlxG.save.data.bloom && Options.gameplayShaders) {ultraCam.addShader(glowShader); camGame.addShader(glowShader2); camHUD.addShader(glowShader2);}
