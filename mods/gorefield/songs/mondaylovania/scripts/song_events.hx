...
function create() {
    ...
    if (FlxG.save.data.bloom && Options.gameplayShaders) FlxG.game.addShader(bloom);

    distortionShader = new CustomShader("chromaticWarp");
    distortionShader.distortion = 0;
    if (FlxG.save.data.warp && Options.gameplayShaders) FlxG.game.addShader(distortionShader);
