...
function goToItem() {
	...
	switch (options[curSelected]) {
		case "story_mode": 
			...
			if (FlxG.save.data.glitch && Options.gameplayShaders) gorefield.shader = glitchShader;
			if (FlxG.save.data.heatwave && Options.gameplayShaders) fire.shader = heatWaveShader;
			if (FlxG.save.data.bloom && Options.gameplayShaders) FlxG.camera.addShader(glowShader);
