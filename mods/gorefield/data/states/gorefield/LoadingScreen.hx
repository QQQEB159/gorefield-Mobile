...
function update(elapsed:Float) {
	if ((FlxG.keys.justPressed.ESCAPE || touchPad.buttonB.justPressed) && skipLoadingAllowed)
		FlxG.switchState(PlayState.isStoryMode ? new StoryMenuState() : new FreeplayState());
	if ((FlxG.keys.justPressed.ENTER || touchPad.buttonA.justPressed) && !pressedEnter && (skipLoadingAllowed == true ? true : finishedLoading)) {goToSong(); pressedEnter = true;}
}
