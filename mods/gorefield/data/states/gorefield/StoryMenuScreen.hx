...
function create() {
	FlxG.mouse.visible = FlxG.mouse.useSystemCursor = !controls.instance.touchC;
	...
	if (FlxG.save.data.warp && Options.gameplayShaders) FlxG.camera.addShader(warpShader);

	chromatic = new CustomShader("chromaticWarp");
    chromatic.distortion = 0; 
    if (FlxG.save.data.warp && Options.gameplayShaders && (weeksUnlocked[5] && !weeksFinished[5])) {camText.addShader(chromatic);}

	chromatic2 = new CustomShader("chromaticWarp");
    chromatic2.distortion = 0; 
    if (FlxG.save.data.warp && Options.gameplayShaders && (weeksUnlocked[5] && !weeksFinished[5])) {FlxG.camera.addShader(chromatic2); camBG.addShader(chromatic2);}
	...
	if (FlxG.save.data.warp && Options.gameplayShaders) camBG.addShader(bgShader);
	...
	if (FlxG.save.data.bloom && Options.gameplayShaders) bgSprite.shader = bloomShader;
...
function update(elapsed:Float) {
	...
	if (canMove && (FlxG.keys.justPressed.TAB || touchPad.buttonT.justPressed) && !codesOpened && FlxG.save.data.canVisitArlene){
		canMove = false;
		codesMenu(false, -100);
...
var codes:Map<String, Void -> Void> = [
	...
	"QQQEB" => CodesFunctions.unlockAll,
