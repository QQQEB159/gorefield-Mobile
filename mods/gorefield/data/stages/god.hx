...
function create() {
    ...
    if (FlxG.save.data.particles && Options.gameplayShaders) particleSprite.shader = particleShader;
    ...
    if(FlxG.save.data.bloom && Options.gameplayShaders) FlxG.camera.addShader(bloom);

    drunk = new CustomShader("drunk");
    drunk.strength = .35; drunk.time = 0;
    if(FlxG.save.data.drunk && Options.gameplayShaders) FlxG.camera.addShader(drunk);

    chromatic = new CustomShader("chromaticWarp");
    chromatic.distortion = 0.2; 
    if (FlxG.save.data.warp && Options.gameplayShaders) FlxG.camera.addShader(chromatic);

    warpShader = new CustomShader("warp");
    warpShader.distortion = 1;
    if (FlxG.save.data.warp && Options.gameplayShaders) camGame.addShader(warpShader);
}

function postCreate() {
    mobileControls.instance.buttonLeft.color = 0x00FFFF;
    mobileControls.instance.buttonDown.color = 0xFFFF00;
    mobileControls.instance.buttonUp.color = 0xFFFF00;
    mobileControls.instance.buttonRight.color = 0x00FFFF;
}
