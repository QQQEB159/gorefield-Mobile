...
function create() {
    ...
    if (FlxG.save.data.heatwave && Options.gameplayShaders) camHUD.addShader(heatWaveShader);
    if (FlxG.save.data.heatwave && Options.gameplayShaders) characterCam.addShader(heatWaveShader);

    bloom = new CustomShader("glow");
    bloom.size = 28.0; bloom.dim = .5;
    if (FlxG.save.data.bloom && Options.gameplayShaders) stage.stageSprites["fireSky"].shader = bloom;
    ...
    if (FlxG.save.data.bloom && Options.gameplayShaders) characterCam.addShader(bloom2);
    if (FlxG.save.data.bloom && Options.gameplayShaders) camGame.addShader(bloom2);
