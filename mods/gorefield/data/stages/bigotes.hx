...
function create() {
    ...
    trailBloom.sat = 1;
    if (FlxG.save.data.bloom && Options.gameplayShaders) camJonTrail.addShader(trailBloom);

    drunkShader = new CustomShader("drunk");
    drunkShader.time = 0; drunkShader.strength = 1;
    if (FlxG.save.data.drunk && Options.gameplayShaders) camJonTrail.addShader(drunkShader);
    ...
    if (FlxG.save.data.particles && Options.gameplayShaders) FlxG.camera.addShader(particleShader);
